...
(deftest test-of-variable
  (let [actual-result (of-variable "someMeasurementType")
        expected-rdf [[:qname :ontology "of_variable"]
                      [:blank
                       '([[:qname :rdf "type"]
                          [:qname :ontology "Variable"]]
                         [[:qname :ontology "measurementType"]
                          [:qname :ontology "someMeasurementType"]])]]]
    (is (= expected-rdf actual-result))))

(deftest test-of-variable-categorical
  (let [category-rdf [[:qname :ontology "categoryList"]
                      [:coll '([:qname :instance "adultsCategoryId"]
                               [:qname :instance "octogenarianCategoryId"])]]
        actual-result (of-variable-categorical category-rdf)
        expected-rdf [[:qname :ontology "of_variable"]
                      [:blank
                       [[[:qname :rdf "type"]
                         [:qname :ontology "Variable"]]
