  ...
  (if (= 0 (count (:categories props)))
    ...
    (if (or (= "Number" (:param props)) (= "Count of Participants" (:param props)))
      (let [category-uris (into {} (map (fn [%] [% (trig/iri :instance (uuid))]) (:categories props)))
            ...
            categories {:uris category-uris :rdfs category-rdfs}]
        [categories (trig/_po [(trig/iri :ontology "measurementType") (trig/iri :ontology "categorical")]
                              [(trig/iri :ontology "categoryList") (trig/coll (vals (:uris categories)))])])
      [nil (trig/_po)])))
...
(defn baseline-var-rdf
  ...
  (let [uri (baseline-uris idx)
        ...
        [categories
         (if (= 0 (count (:categories props)))
           (-> subj
               (trig/spo [(trig/iri :ontology "has_result_property") (trig/iri :ontology "sample_size")])
               (spo-each (trig/iri :ontology "has_result_property")
                         (map #(trig/iri :ontology %) (keys properties))))
           subj)]))
...
(defn find-adverse-event-time-frames
  [xml]
  { [:events] (or 
    (vtd/text (vtd/at xml "/results/adverseEvents/adverseEvent/timeFrame")) 
    "Unknown") })

(defn find-endpoint-time-frame
  [xml]
...
(defn find-measurement-moments
  [xml]
  (sort-equivalent-values (merge (find-endpoint-time-frame xml)
                                 (find-adverse-event-time-frames xml)
                                 {[:baseline] "Baseline"})
                                 lower-case))
...
(defn ctgov-import
  ...
  (let [prefixes {:rdf "http://www.w3.org/1999/02/22-rdf-syntax-ns#"
        ...
        event-xml (vtd/search xml "/clinical_study/clinical_result/reported_events/*//category_list/category/event_list/event")
        ...
        baseline-xml (vtd/search xml "/clinical_study/clinical_result/baseline/measure_list/measure")
        baseline-sample-size-xml (vtd/at xml "/clinical_study/clinical_result/baseline/analyzed_list/analyzed")
